...
guide for every coding agent; deeper detail lives in `docs/` (linked at the bottom). Claude Code
reads it via an `@AGENTS.md` import in `CLAUDE.md`. Put project guidance here, not there.
...
```bash
bin/dev                       # foreman: web + sidekiq + JS/CSS watchers. Needs a TTY; agents don't use it
...
## Running agents in parallel (worktrees)

Several agents work this repo at once, each in its own git worktree. Test databases are already
isolated per checkout (see above) — `bin/rails test` is safe to run concurrently.

**Starting the app: `yarn build:all`, then `bin/rails server`.** Not `bin/dev` — it needs a TTY,
and its Sidekiq and file-watcher processes are not wanted for a test run.

**Port 3000 is shared, and this silently corrupts E2E results.** Caddy proxies every dev hostname
to `localhost:3000` regardless of which worktree is listening, and Playwright targets those
hostnames. If another worktree holds the port, your E2E run tests ITS code and reports the result
as yours — green when you are broken, red when you are fine, with nothing in the output to show
it. Before `yarn test:e2e`, confirm the port is yours:
...
If it prints another checkout, stop and tell the user — do not kill their server, and do not run on
