...
`is_dangling` is NULL, not `false`, for the 115,965 `edition` and `other`
redirects: nothing checks those targets against a table, so claiming they
resolve would be an assertion nobody made. Only `work` (2,573) and `author`
(783) redirects are actually resolved and can be `true`.
...
`data-sources/tests/fixtures/` is a 735-line sample of the real dumps,
