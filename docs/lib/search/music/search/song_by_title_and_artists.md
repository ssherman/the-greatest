...
## Summary
OpenSearch search class for finding songs by structured title and artists parameters. Designed for high-precision matching to prevent duplicate song imports during items_json enrichment.

## Purpose
This search class provides structured, high-precision song matching by accepting separate title and artists parameters (rather than free-text search). It's used by the items_json enricher to find existing songs in our local database before querying external APIs like MusicBrainz.

## Key Differences from SongGeneral
...
### `.index_name`
Delegates to `Search::Music::SongIndex.index_name`
- Returns: String - The OpenSearch index name for songs

### `.call(title:, artists:, **options)`
Searches for songs matching the given title and artists.
...
### Items JSON Enrichment
Primary use case - finding existing songs before MusicBrainz search:
```ruby
...
- Uses existing `Search::Music::SongIndex`
...
- `Search::Music::SongIndex` - Song OpenSearch index
- `Search::Shared::Utils` - Query building utilities

## Related Classes
- `Search::Music::Search::SongGeneral` - Free-text song search (lower precision, broader use)
- `Services::Lists::Music::Songs::ItemsJsonEnricher` - Primary consumer
- `Search::Music::SongIndex` - Index definition with `title` and `artist_names` fields

## Testing
Comprehensive test coverage in `test/lib/search/music/search/song_by_title_and_artists_test.rb`:
...
- **Title Required**: Can't match a song without knowing the title
- **Artist Filtering**: At least one artist must match for confidence
- **Multi-Artist Support**: Any artist match is sufficient for songs with multiple artists
...
- **Existing Data Quality**: Our indexed songs are already vetted
- **Configurable**: Can be lowered for specific use cases

## Future Enhancements
- Batch search support for multiple songs
- Confidence scoring (high/medium/low ranges)
- Match validation with machine learning
