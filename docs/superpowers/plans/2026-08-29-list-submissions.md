...
5. **Delete the `corrections--form` alias, once the edge cache has cycled past this deploy (>24h).** Generalizing the correction token endpoint to `/form_token` (so list submission could reuse it) left a deliberate compatibility shim behind: cached correction-form HTML from before this deploy still carries `data-controller="corrections--form"` and still requests `/correction_token`, so both old names have to keep working until every copy of that cached page has expired. Three places carry it, and none of them expire on their own:
   - `application.register("corrections--form", ...)` in `web-app/app/javascript/manifests/web_shared.js` — the Stimulus alias itself.
   - the `DEPRECATED_ALIASES` entry in `web-app/test/lint/stimulus_manifest_test.rb` — the one thing keeping the manifest lint from flagging that alias as unreferenced.
   - the `get "correction_token"` route in `web-app/config/routes.rb` — keeps stale cached pages' token fetch resolving instead of 404ing.

   Once 24h have passed since this deploy, remove all three in one PR: delete the registration line, delete the `DEPRECATED_ALIASES` entry (and drop the now-unneeded exception logic in the lint test if nothing else uses it), and delete the `correction_token` route. Leaving them in place after the cache has cycled costs nothing but a little clarity; removing them before it has cycled breaks the Add/Remove alternate-titles buttons and CSRF-token attribution on any correction-form page a visitor still has cached.
