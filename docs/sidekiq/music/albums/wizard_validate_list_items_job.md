...
## Summary
Sidekiq background job that validates enriched ListItems using AI. Part of the Album List Wizard Step 4 (Validate).
...
1. Load list and identify enriched items (items with `listable_id`, `album_id`, or `mb_release_group_id`)
...
```json
{
  ...
  "metadata": {
    ...
    "verified_count": 45,
    "reasoning": "...",
    "validated_at": "2025-12-28T15:30:00Z"
  }
}
```
...
### `#has_enrichment?(item)`
Checks if an item has enrichment data (`listable_id`, `album_id`, or `mb_release_group_id`).
...
Music::Albums::WizardValidateListItemsJob.perform_async(list.id)
...
- `Music::Albums::List` with `list_items` association
- `List#wizard_manager` for step status updates

## Related Files
- `app/lib/services/ai/tasks/lists/music/albums/list_items_validator_task.rb` - AI task
- `app/controllers/admin/music/albums/list_wizard_controller.rb` - Controller that enqueues job
- `app/components/admin/music/albums/wizard/validate_step_component.rb` - UI component
- `app/sidekiq/music/albums/wizard_enrich_list_items_job.rb` - Similar job pattern (enrich step)
- `app/sidekiq/music/songs/wizard_validate_list_items_job.rb` - Songs equivalent
