class Admin::ListItemsController < Admin::BaseController
  ...
  rescue ListItem::AutoGeneratedListItemError => e
    # The generated list's items are read-only in admin and the buttons are
    # hidden for it, but a stale tab or a bookmarked URL still reaches here.
    # Refuse the way create/update already do, rather than 500.
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.replace(
          "flash",
          partial: "admin/shared/flash",
          locals: {flash: {error: e.message}}
        ), status: :unprocessable_entity
      end
      format.html do
  ...
  def clear_positions
    ...
    # sees this action -- it would silently wipe positions the generator only
    # restores on the next nightly run. This is the only guard it gets.
    if @list.auto_generated?
      return redirect_to redirect_path,
        alert: "Positions on an auto-generated list are managed by the generator and cannot be cleared."
    end
