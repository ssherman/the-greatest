...
# Used by any domain's ListWizardController (Music, Games, etc.).
#
# == Wizard Steps
#
# 1. *source* - Select import source
# 2. *parse* - Parse HTML to extract items
# 3. *enrich* - Enrich items with external data
...
#   - load_review_step_data: Load data for review step (domain-specific eager loading)
#   - load_import_step_data: Load data for import step (domain-specific queries)
#
# Subclasses may optionally override:
#   - valid_import_sources: Array of valid import source types (default: ["custom_html"])
#   - load_source_step_data, load_parse_step_data, load_enrich_step_data, etc.
#
module BaseListWizardController
  ...
  # Override in subclasses to add additional valid import sources
  ...
  # Default step data loaders - override in subclasses as needed
  ...
  # Subclasses must implement - domain-specific eager loading
  def load_review_step_data
    raise NotImplementedError, "Subclass must implement #load_review_step_data"
  end

  # Subclasses must implement - domain-specific import queries
  def load_import_step_data
    raise NotImplementedError, "Subclass must implement #load_import_step_data"
  end
  ...
  def advance_from_source_step
    ...
    next_step_index = source_step_next_index(import_source)
  ...
  # Override in subclasses to customize which step to go to after source selection
  def source_step_next_index(import_source)
    1 # default: go to parse step
  end
