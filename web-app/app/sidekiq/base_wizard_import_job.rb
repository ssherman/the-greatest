...
# Imports entities from external APIs and links them to list items.
#
...
#   - importer_params(external_id): Hash of params for importer call
...
#   - series_import_source_name: Name of the series import source (default: "musicbrainz_series")
#
class BaseWizardImportJob
  include Sidekiq::Job
  ...
  def perform(list_id)
    ...
    if import_source == series_import_source_name
      import_from_series
    else
  ...
  def importer_params(external_id)
  ...
  # Override in subclasses to change series import source name (default: "musicbrainz_series")
  def series_import_source_name
    "musicbrainz_series"
  end
  ...
  def import_from_series
    @list.wizard_manager.update_step_status!(step: "import", status: "running", progress: 0, metadata: {
      "import_source" => series_import_source_name
    })

    # Default: fall back to importing items that have external data
    @items_to_import = items_needing_import
    @total = @items_to_import.count

    if @total.zero?
      complete_with_no_items(series_import_source_name)
      return
    end
    ...
    @items_to_import.each_with_index do |item, index|
      import_item(item)
      update_progress(index + 1, series_import_source_name) if should_update_progress?(index)
    end

    complete_job(series_import_source_name)
  end
  ...
  def import_item(item)
    external_id = item.metadata[enrichment_id_key]

    begin
      result = importer_class.call(**importer_params(external_id))
