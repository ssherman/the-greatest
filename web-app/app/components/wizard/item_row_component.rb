...
#   - source_badge: Hash with :text, :css_class, :title for match source
...
#
class Wizard::ItemRowComponent < ViewComponent::Base
  def initialize(item:)
    @item = item
  end
  ...
  def source_badge
    raise NotImplementedError, "Subclass must implement #source_badge"
  end
