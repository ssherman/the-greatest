class DefaultUserListsToGridView < ActiveRecord::Migration[8.1]
  ...
  # restores the column default only. That's true of the ~282,687 development
  # rows; in production the ~440 pre-existing rows (music/games/movies, where 0
  # was simply the old default) can't be distinguished this way either, so a
  # user who deliberately chose "List" looks identical to one who never chose —
  # accepted because the population is tiny and the preference is one click to
  # restore.
  def up
