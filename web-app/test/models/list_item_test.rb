...
class ListItemTest < ActiveSupport::TestCase
  ...
  test "refuses to destroy an item on an auto-generated list" do
    list = lists(:basic_list)
    item = ListItem.create!(list: list, listable: books_books(:war_and_peace), position: 1)
    list.update!(auto_generated_kind: :user_favorites)

    error = assert_raises(ListItem::AutoGeneratedListItemError) { item.destroy }
    assert_includes error.message, "managed by the generator"
    assert ListItem.exists?(item.id)
  end

  test "refuses a scoped destroy_all on an auto-generated list's items" do
    list = lists(:basic_list)
    item = ListItem.create!(list: list, listable: books_books(:war_and_peace), position: 1)
    list.update!(auto_generated_kind: :user_favorites)

    assert_raises(ListItem::AutoGeneratedListItemError) do
      list.list_items.where(id: item.id).destroy_all
    end
    assert ListItem.exists?(item.id)
  end
