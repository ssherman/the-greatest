...
class ListTest < ActiveSupport::TestCase
  ...
  test "destroying a normal list with items succeeds" do
    list = lists(:basic_list)
    item = ListItem.create!(list: list, listable: books_books(:war_and_peace), position: 1)

    list.destroy!

    refute List.exists?(list.id)
    refute ListItem.exists?(item.id)
  end
  ...
  test "refuses to destroy an auto-generated list" do
    ...
    error = assert_raises(List::AutoGeneratedListError) { list.destroy }

    assert_includes error.message, "managed by the generator"
    assert List.exists?(list.id)
  end

  test "refuses to destroy an auto-generated list through destroy!" do
    list = lists(:basic_list)
    list.update!(auto_generated_kind: :user_favorites)

    assert_raises(List::AutoGeneratedListError) { list.destroy! }
    assert List.exists?(list.id)
  end

  test "refuses a scoped destroy_all that would take an auto-generated list" do
    list = lists(:basic_list)
    list.update!(auto_generated_kind: :user_favorites)

    assert_raises(List::AutoGeneratedListError) do
