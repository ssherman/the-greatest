...
module SavedSearches
  class LanguagesControllerTest < ActionDispatch::IntegrationTest
    ...
    test "returns the autocomplete shape" do
      ...
      get saved_search_languages_path(q: "russ"), as: :json

      assert_response :success
      row = response.parsed_body.find { |r| r["value"] == languages(:russian).id }
      assert_equal "Russian", row["text"]
    end

    test "returns nothing for a blank query" do
      sign_in_as(@user, stub_auth: true)

      get saved_search_languages_path(q: ""), as: :json
