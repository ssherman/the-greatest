...
module Music
  class Artist
    class MergerTest < ActiveSupport::TestCase
      ...
      test "should reassign band_memberships to target artist" do
        member = music_artists(:david_bowie)
        membership = Music::Membership.create!(
          artist: @source_artist,
          member: member,
          joined_on: Date.new(2000, 1, 1)
        )
        ...
        assert_equal @target_artist.id, membership.artist_id
      end

      test "should handle duplicate band_memberships by destroying source" do
        member = music_artists(:david_bowie)

        Music::Membership.create!(
          artist: @source_artist,
          member: member,
          joined_on: Date.new(2000, 1, 1)
        )
        ...
        initial_count = @target_artist.band_memberships.count
        ...
        assert_equal initial_count, @target_artist.band_memberships.count
      end

      test "should reassign memberships (as member) to target artist" do
        # Create fresh artists to avoid fixture conflicts
        source_person = Music::Artist.create!(name: "Source Member", slug: "source-member-test", kind: :person)
        target_person = Music::Artist.create!(name: "Target Member", slug: "target-member-test", kind: :person)
        band = Music::Artist.create!(name: "Test Band", slug: "test-band-merger", kind: :band)

        membership = Music::Membership.create!(
          artist: band,
          member: source_person,
          joined_on: Date.new(2000, 1, 1)
        )

        result = Music::Artist::Merger.call(source: source_person, target: target_person)

        assert result.success?
        membership.reload

        assert_equal target_person.id, membership.member_id
      end

      test "should reassign credits to target artist" do
        song = music_songs(:money)
        credit = Music::Credit.create!(
          artist: @source_artist,
          creditable: song,
          role: :writer
        )
        ...
        assert_equal @target_artist.id, credit.artist_id
      end
