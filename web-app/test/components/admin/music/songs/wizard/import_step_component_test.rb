...
class Admin::Music::Songs::Wizard::ImportStepComponentTest < ViewComponent::TestCase
  setup do
  ...
  test "renders based on import_source in wizard_state" do
    ...
    render_inline(Admin::Music::Songs::Wizard::ImportStepComponent.new(list: @list))

    assert_text "Import songs from MusicBrainz based on matched recordings"
  end
  ...
  test "custom_html: renders stats cards with correct counts" do
    @list.list_items.first.update!(listable_id: music_songs(:time).id)
  ...
  test "custom_html: shows failed items section when failures exist" do
    set_import_state(
      ...
      metadata: {
        ...
        "failed_count" => 1,
        "errors" => [
          {"item_id" => 1, "title" => "Failed Song", "error" => "Recording not found"}
        ]
      }
    )
    ...
    assert_text "Recording not found"
  end
  ...
  test "series: shows songs imported and list items created counts" do
    @list.update!(musicbrainz_series_id: "abc123-series-mbid")
    set_import_state(
      import_source: "musicbrainz_series",
      status: "completed",
    ...
    assert_selector ".stat-title", text: "Songs Imported"
