...
// the stat labels differ only in the media noun. Music is the only domain that
// passes two ranking configurations, so its noun reads "albums and songs".
test.describe('Music Rankings Page', () => {
  ...
  test('rankings page displays stats section', async ({ page }) => {
    ...
    await expect(stats.getByText('Ranked albums and songs')).toBeVisible();
