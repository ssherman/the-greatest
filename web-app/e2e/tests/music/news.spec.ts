...
// The music and games projects both run authenticated (storageState in
...
// development has ZERO music news posts, so the plan's `test.skip(count === 0)`
...
const E2E_PREFIX = "E2E Music News";
...
async function createPublishedPost(page: Page, title: string) {
  ...
  await page.locator('textarea[name="news_post[body]"]').fill(
    "A **bold** claim from the music E2E spec."
  );
...
test.describe("Music news", () => {
  ...
  test("the index advertises its feed and the feed serves this site", async ({ page }) => {
    ...
    await expect(page.locator('link[rel="alternate"][type="application/rss+xml"]')).toHaveAttribute(
      "href",
      "https://dev.thegreatestmusic.org/news.rss"
    );
